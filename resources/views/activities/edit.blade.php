            ...
            <div class="bg-white shadow-sm sm:rounded-lg border border-gray-100 p-6">
                <form method="POST" action="{{ route(auth()->user()->role . '.activities.update', $activity) }}">
                    ...
                    <div class="flex items-center justify-end mt-4 text-right">
                        <a href="{{ route(auth()->user()->role . '.activities.show', $activity) }}" class="text-sm text-secondary hover:text-primary rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary mr-4 font-semibold">
                            Cancelar
                        </a>
                        <x-primary-button>
                            Atualizar Atividade
